# -*- coding: utf-8 -*-
"""
Created on Sat Dec 21 09:02:58 2019
...
def get_most_freq_words(data, n_freq_words):
    counter = Counter()
...
def remove_freq_words(text):
    n_freq_words = 10
    FREQ_WORDS = get_most_freq_words(data,n_freq_words)
    return " " .join([word for word in str(text).split() if word not in FREQ_WORDS])

def get_most_rare_words(data, n_rare_words):
    counter = Counter()
    for text in data['Review'].values:
        for word in text.split():
            counter[word] += 1
...
def remove_rare_words(text):
    n_rare_words = 10
    RARE_WORDS = get_most_rare_words(data,n_rare_words)
...
def remove_html_tags_with_re(text):
    html_pattern = re.compile('<.*?>')
    return html_pattern.sub(r'', text)

def remove_html_tags_with_BeautifulSoup(text):
    return BeautifulSoup(text, "lxml").text

def get_chat_words_list_dict(CHAT_WORDS):
    chat_words_map_dict = {}
    chat_words_list = []
